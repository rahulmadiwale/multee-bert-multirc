...
@Model.register("bert-split-glove")
class BertSplitGlove(Model):

    def __init__(self, vocab: Vocabulary,
                 first_stage_count) -> None:
        super(BertSplitGlove, self).__init__(vocab)
    ...
    def forward(self,
        ...
        for key in premises.keys():
            premises[key] = torch.squeeze(premises[key])
...
@Model.register("bert-multee-glove")
class MulteeBertGloveAttention(Model):

    def __init__(self, vocab: Vocabulary,
                 attention_module: Model,
                 classifier: FeedForward) -> None:
        super(MulteeBertGloveAttention, self).__init__(vocab)

        self.attention_module = attention_module
        self.branch_1 = BertSplitGlove(vocab, 4)
        self.branch_2 = BertSplitGlove(vocab, 6)
    ...
    def forward(self,
                ...
                attention_premises: Dict[str, torch.LongTensor],
                attention_hypotheses: Dict[str, torch.LongTensor],
        ...
        for i in range(num_hypothesis):
            attention_module_output = self.attention_module(attention_premises,
                                                            {"tokens": attention_hypotheses["tokens"][:, i, :]},
            ...
            if loss_ is not None:
                if attention_loss is None:
                    ...
                    attention_loss += loss_
        print(premises['tokens'].shape, attentions[0].shape)
        concatenated_pooled = []
        for i in range(num_hypothesis):
...
#                 relevance_presence_mask: torch.IntTensor = None) -> Dict[str, torch.Tensor]:
#
#         print(premises["tokens"])
#         print("ATT premise ", attention_premises["tokens"])
#         print("HYPOTHESIS ", hypotheses["tokens"])
#         print("ATTN hypo" , attention_hypotheses["tokens"])
#
#         num_hypothesis = hypotheses["tokens"].shape[1]
#         attentions, attention_loss = [], None
#         for i in range(num_hypothesis):
...
#                 else:
#                     attention_loss += loss_
#         print("CHECK THIS" * 20, attentions[0].shape, premises['tokens'].shape)
#         concatenated_pooled = []
#         for i in range(num_hypothesis):
#             branch_1_embeddings = self.branch_1(premises, attentions[i])
#             branch_2_embeddings = self.branch_2(premises, attentions[i])
