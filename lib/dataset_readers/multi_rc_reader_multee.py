...
@DatasetReader.register("multiple_correct_mcq_multee")
class MultipleCorrectMcqEntailmentReaderMultee(DatasetReader):
        ...
        with open(file_path, 'r') as entailment_file:
            ...
            for line in entailment_file:
                if line.strip():
                    ...
                    question = instances_json["raw_question"]
    ...
    def text_to_instance(self, # pylint: disable=arguments-differ
        ...
        fields = {}
        premises_tokens = [list(map(Token, self._tokenizer(premise)[-self._premise_max_tokens:]))
                           for premise in premises]
        hypotheses_tokens = [list(map(Token, self._tokenizer(hypothesis)[-self._hypothesis_max_tokens:]))
